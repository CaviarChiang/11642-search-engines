 ...
 *      int externalID = q.docIteratorGetMatch ();
 *      double score = ((QrySop) q).getScore (model);
 *      System.out.println ("internal externalID: " + externalID + ", score: " score);
 *      q.docIteratorAdvancePast (externalID);
 *    }
 *  </pre>
 *  <p>
...
public abstract class Qry {
    ...
    // INVALID_DOCID, ArrayList<Qry> args, dispName, docIteratorMatchCache, matchStored, matchingDocid
    ...
    /**
     *  The arguments to this query operator.
     *  The TERM query operator has 0 arguments.
     *  The SCORE query operator has 1 argument.
     *  All other query operators have 1 or more arguments (e.g., #SYN (blue aqua)).
    ...
    /**
     *  docIteratorHasMatch caches the matching externalID so that
    ...
    public void appendArg(Qry q) throws IllegalArgumentException {
        ...
        if (this instanceof QrySopScore) {
            if (this.args.size() > 0) { // if QrySopScore already has a arg, then you cannot add it to QrySopScore args anymore
                throw new IllegalArgumentException
        ...
        if ((this instanceof QryIop) && (q instanceof QryIop)) {
            ...
            if (this.args.size() == 0) { // QryIop has no args
    ...
    /**
     *  Clear the docIterator's matching externalID cache.  The cache should
    ...
    public int docIteratorGetMatch() {
        if (this.docIteratorHasMatchCache()) {
            ...
            throw new IllegalStateException("No matching externalID was cached.");
        }
    }
    ...
    protected boolean docIteratorHasMatchAll(RetrievalModel r) {
        ...
        while (!matchFound) {
            ...
            // Get the externalID of the first query argument.
            ...
            // Other query arguments must match the externalID of the first query
            // argument.
     ...
     *  the match is the smallest externalID to match; some subclasses may
    ...
    /**
     *  Set the matching externalID cache.
