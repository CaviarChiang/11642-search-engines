...
public class InvList {
   ...
   */
  public class DocPosting { // DocPosting(int externalID, int... locations) | DocPosting(int externalID, List<Integer> locations) (always initialize tf)
    ...
    public DocPosting(int d, List<Integer> locations) {
      ...
      for (int j = 0; j < locations.size(); j++)
        this.positions.add(locations.get(j));
    }
  }
   ...
   */
  public InvList() { // ctf, df, field, postings
  }
   ...
   *  in externalID order, otherwise this method fails.
  ...
  public boolean appendPosting (int docid, List<Integer> positions) {
    ...
    //  A posting can only be appended if its externalID is greater than
    //  the last externalID.
  ...
  public void print() {
    ...
    for (int i = 0; i < this.df; i++) {
      System.out.print("externalID:  " + this.postings.elementAt(i).docid + ", tf: "
