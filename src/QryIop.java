...
public abstract class QryIop extends Qry {
    ...
    /**
     * Get the score for a given externalID associated with this query operator.
     * @param r Retrieval model instance.
    ...
    /**
     * Get the score for a given externalID associated with this query operator, based on Ranked Boolean model
     * @param r Retrieval model instance. Assume RetrievalModelRankedBoolean
     * @return Document score.
     */
    public double getScoreRankedBoolean(RetrievalModel r) {
        // return document's term frequency as score
        return this.invertedList.getTf(docIteratorIndex);
    }

    /**
     * Get the score for a given externalID associated with this query operator, based on BM25 model
     * @param r Retrieval model instance. Assume RetrievalModelBM25
     * @return Document score.
     */
    public double getScoreBM25(RetrievalModel r) throws IOException {
        ...
        double df = this.getDf();
        double tf = this.invertedList.getTf(docIteratorIndex);
        // double tf = this.docIteratorGetMatchPosting().tf;
        double qtf = 1;
        ...
        // Calculation
        rsjWeight = Math.max( 0, Math.log( (numDocs - df + 0.5) / (df + 0.5) ) );
        tfWeight = tf / ( tf + k1 * ( (1 - b) + b * docLen / avgDocLen ) );
        userWeight = (k3 + qtf) / (k3 + qtf);
    ...
    /**
     * Get the score for a given externalID associated with this query operator, based on Indri model
     * @param r Retrieval model instance. Assume RetrievalModelIndri
     * @return Document score.
     */
