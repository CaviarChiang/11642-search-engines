...
public class QryParser {
  ...
  private static Qry createOperator (String operatorName) {
    ...
    if (operatorNameLowerCase.contains("#near/")) {
        operatorDistance = Integer.parseInt(operatorNameLowerCase.split("/")[1].trim());
        operator = new QryIopNear(operatorDistance);
    }

    else if (operatorNameLowerCase.contains("#window/")) {
        operatorDistance = Integer.parseInt(operatorNameLowerCase.split("/")[1].trim());
  ...
  private static Qry[] createTerms (String token) throws IOException {
    ...
    for (int i = 0; i < t.length; i++) {
      terms[i] = new QryIopTerm(t[i], field);
    }
  ...
  private static Qry parseString (String queryString)
    ...
    while (queryString.length() > 0) {
      ...
      //  #WAND (0.5 #AND (Pearl farming) 0.3 #AND (#NEAR/5 (Pearl farming)) 0.2 #AND (#WINDOW/8 (Pearl farming)))
  ...
  static private PopData<Double, String> popWeight (String argString) {
      ...
      double weight = Double.parseDouble(argString.substring(start, end + 1).trim());
      argString = argString.substring(0, start) + argString.substring(end + 1);

      return new PopData<>(weight, argString);
  ...
  static private PopData<String,String> popTerm (String argString) {
    ...
    return new PopData<String,String>(substrings[0], argString); // PopData(PopType popped, RemainingType remaining)
  }
  ...
  public static String[] tokenizeString(String query) throws IOException {
    ...
    public static void main(String[] args) throws IOException {
        ...
        String query = "#WAND (0.5 #WAND (0.3 Pearl 0.7 farming) 0.3 #AND (#NEAR/5 (Pearl farming)) 0.2 #AND (#WINDOW/8 (Pearl farming)))";
        System.out.println(parseString(query));
